...
// v2.3 — fixes: dropdown scroll-infinito, posição botão, injeção iframes
...
(function () {
  ...
  function createButtonInstance(rootId, isPreview) {
    ...
    css.textContent = `
      ...
      #${ID('arr')} {
        width: 5.0px !important;
        height: 3.58px !important;
  ...
  function classifyPlayers() {
    ...
    if (pv && !pv.classList.contains("idm-closed") &&
        !pv.classList.contains("idm-hidden") && mouseX >= 0) {
    ...
    // de anúncio) podem ter dezenas de elementos <video>. Processar
    // todos a cada 400ms (preview) com getClippedRect — que percorre até
    // 6 ancestrais com getComputedStyle cada — é custoso e, combinado
    // com estruturas DOM profundas/atípicas de alguns desses sites, foi
    // associado a um RangeError (Maximum call stack size exceeded) e a
    // lentidão perceptível na página. Limitamos a varredura aos 30
    // primeiros <video> encontrados — nenhum site real de streaming tem
    // mais que isso de candidatos legítimos a principal/preview.
    const allVideos = document.querySelectorAll("video");
    const videoCap  = Math.min(allVideos.length, 30);
  ...
  function startPlayerWatcher(wrap, closeDrop, onNavigate, isPreviewInstance) {
    ...
    if (isPreviewInstance) {
      function previewTick() {
        ...
        // preview válido e principal ativo — garantir que a instância
        // está pronta e exibida.
        dualButtonCoordinator.ensureInstance();
        if (lastHidden !== false) {
